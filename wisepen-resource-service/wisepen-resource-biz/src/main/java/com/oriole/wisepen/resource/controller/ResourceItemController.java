...
public class ResourceItemController {
    ...
    @Operation(
            ...
            description = """
                    ...
                    - 处理：个人空间默认排除个人回收站体系下的资源，除非显式传入回收站内标签；小组空间按当前用户在小组中的角色和资源 ACL 查询可见资源，并批量补充当前标签名、互动统计及 offerInfo（买家仅见已审核通过且在售条目，资源所有者与小组管理员可见全部）；不返回当前用户无权查看的资源。
                    - 失败：未登录 -> PermissionError.NOT_LOGIN；当前用户不属于目标小组 -> PermissionError.PERMISSION_DENIED。
                    - 响应：返回分页资源列表、总数、当前页资源的标签名映射、互动统计及 offerInfo。
                    """
    )
